...
>
  <div class="max-w-2xl mx-auto">
    ...
    <!-- 読み込み状態 (使用深藍色) -->
    {#if pending}
      <div class="text-center py-20 text-blue-800 font-black">
        <p class="animate-pulse">読み込み中...</p>
      </div>
    {:else}
      <!-- リスト表示 -->
      <div class="space-y-3 mt-8">
        {#each filteredSongs as song, index}
          ...
          {#if index === 0 || song.Date !== filteredSongs[index - 1]?.Date}
            <div
              class="sticky top-[90px] z-10 bg-blue-50/95 py-3 border-b-2 border-blue-100 mb-2"
            >
          ...
          >
            ...
            <div class="flex-1 min-w-0 ml-3">
              <div class="flex flex-col">
                <!-- 歌曲標題：深色 Slate-900 -->
                <h3
                  class="text-base font-black text-slate-900 truncate group-hover:text-blue-700 transition-colors"
                >
                  {song.Title}
                </h3>
                <!-- 歌手名稱：調深至 Blue-700 -->
                <span
                  class="text-xs font-bold text-blue-700 mt-1 truncate uppercase tracking-tight"
                  >{song.Artist}</span
                >
              </div>
            </div>
